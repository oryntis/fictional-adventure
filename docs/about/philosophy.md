...
- **Trust**: No hidden backdoors or supply chain attacks (Ken Thompson’s _Trusting Trust_ problem).
...
_(Non-negotiable rules that define the project)_

### **1. Speed First**

&gt; _"Every subsystem must be measurably faster than Linux equivalents on the same hardware. No exceptions."_
...
&gt; _"The OS must run fully functional on 1GB RAM. Idle RAM usage target: **&lt;150MB**."_
...
&gt; _"Secure behavior is the DEFAULT — users must opt OUT of security, not opt IN."_
...
&gt; _"Must support **x86-64, ARM64, and RISC-V** from day one. No proprietary hardware lock-in."_
...
&gt; _"First-class support for **CPU + GPU + NPU + TPU** as equal compute citizens."_
...
&gt; _"Network stack built on **zero-trust principles**. No open ports by default."_
...
&gt; _"Power usage must be **measurable, reportable, and minimizable** at every layer."_
...
&gt; _"All drivers run in **isolated userspace**. A bad driver cannot crash the kernel."_
...
&gt; _"No 30-year-old syscall baggage. **Capability-based interface** from day one."_
...
_(How Orion OS rethinks fundamental OS design)_
...
_(How the philosophy translates to real-world decisions)_
...
- **Problem**: You cannot write a compiler without a compiler (**Ken Thompson’s _Trusting Trust_**).
